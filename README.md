...
DJ controls include `cap!voice list`, `cap!voice dry`, `cap!voice preview cozy`, `cap!personality quirky`, `cap!personality roast`, `cap!talk less`, `cap!shutup`, and `cap!intro`. By default, Cappy opens each listening session and returns after a rotating 2–4 songs with a varied 70–110-word transition or recap. Roast personality comments on and playfully roasts every song. A fresh bot start, an explicit stop/leave, or returning after playback and the queue have gone idle starts a new spoken session when chatter is enabled. DJ copy may draw on unofficial music lore, rumors, genre knowledge, and informed interpretation for insight and comedy; uncertain lore is treated playfully rather than as confirmed reporting. The explicit `cap!fact` command remains provenance-backed. `cap!volume 0-100` adjusts the shared stream; each listener can privately right-click CappyFM in voice and use Discord's User Volume slider. New sessions start at a conservative 60% shared volume.
...
Spotify's playlist-items API requires user authorization and may only expose playlists owned by or collaboratively shared with that user. Add `http://127.0.0.1:8888/callback` to the Spotify app's redirect URIs, then run `./run spotify-login` (`run.cmd spotify-login` on Windows) once on each computer that hosts CappyFM. Open the URL printed by the launcher and approve `playlist-read-private`. CappyFM stores the refresh token at `data/spotify-refresh-token`, which is ignored by Git and mounted only into the bot container. Access tokens are refreshed automatically. Use `./run spotify-status` or `run.cmd spotify-status` to verify that the local machine has credentials and authorization. If that authorized request rejects a playlist, CappyFM falls back to catalog extraction when available and reassembles the recovered track list from YouTube.

Apple Music song, album, and catalog-playlist links are supported through LavaSrc. Add an Apple Music developer token as `APPLE_MUSIC_API_TOKEN` in `.env`, then restart both CappyFM and Lavalink for the most reliable access. Apple Music supplies catalog metadata; protected Apple audio is not streamed directly, so each recovered playlist item is matched to YouTube playback just like Spotify metadata. Use `./run apple-music-status` or `run.cmd apple-music-status` to verify the launcher can see the token.
